...
====================
PURPOSE:
  Central place for all ASTRO settings: API keys, paths, model names,
  and the ASTRO system prompt. Designed for single-user use: each person runs
...
WHAT THIS FILE DOES:
  ...
  - Holds the full system prompt that defines ASTRO's personality and formatting rules.
 USAGE:
  Import what you need: `from config import GROQ_API_KEY, CHATS_DATA_DIR, ASTRO_SYSTEM_PROMPT`
...
# ============================================================================
# BRAIN MODEL (Query Classification — Astro Mode)
# ============================================================================
...
# Default: en-GB-RyanNeural (male British voice, fitting for ASTRO).
...
# ============================================================================
# ASTRO PERSONALITY CONFIGURATION
# ============================================================================
...
# Assistant name and user title: set ASSISTANT_NAME and ASTRO_USER_TITLE in .env.
# The AI learns from learning data and conversation history.

ASSISTANT_NAME = (os.getenv("ASSISTANT_NAME", "").strip() or "Astro")
ASTRO_USER_TITLE = os.getenv("ASTRO_USER_TITLE", "").strip() or os.getenv("JARVIS_USER_TITLE", "").strip()
# Owner's name (e.g. Shreshth) — used so the AI knows who it serves; helps to avoid confusing itself with other AIs
ASTRO_OWNER_NAME = os.getenv("ASTRO_OWNER_NAME", "").strip() or os.getenv("JARVIS_OWNER_NAME", "").strip()

_ASTRO_SYSTEM_PROMPT_BASE = """You are {assistant_name}, a complete AI assistant — not just a chat bot. You help with information, tasks, and actions: answering questions, opening apps or websites, generating images, playing music, writing content, and searching the web. You are sharp, warm, and a little witty. Keep language simple and natural.
...
_ASTRO_SYSTEM_PROMPT_BASE_FMT = _ASTRO_SYSTEM_PROMPT_BASE.format(assistant_name=ASSISTANT_NAME)
if ASTRO_USER_TITLE:
    ASTRO_SYSTEM_PROMPT = _ASTRO_SYSTEM_PROMPT_BASE_FMT + f"\n- When appropriate, you may address the user as: {ASTRO_USER_TITLE}"
else:
    ASTRO_SYSTEM_PROMPT = _ASTRO_SYSTEM_PROMPT_BASE_FMT
