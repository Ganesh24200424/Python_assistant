# ASTRO - Just A Rather Very Intelligent System

An intelligent AI assistant built with FastAPI, LangChain, Groq AI, and a modern glass-morphism web UI. ASTRO provides three chat modes (Astro, General, Realtime), streaming responses, text-to-speech, voice input, and learns from your personal data files. Everything runs on one server with one command.
...
ASSISTANT_NAME=Astro
ASTRO_USER_TITLE=Sir
ASTRO_OWNER_NAME=YourName
...
- **Astro Mode**: Unified route that auto-classifies each query. A Groq-powered brain decides "general" or "realtime" in ~150–400 ms, then routes accordingly. Brain and chat use different API keys (rotation).
...
- **Astro**: `POST /chat/astro/stream` (auto-routes to general or realtime)
...
1. **Query Extraction**: A fast LLM call (with `max_tokens=50`, `temperature=0`) converts the user's raw conversational text into a clean search query. Example: "tell me about that website I mentioned" becomes "Astro knowledge base website". It uses the last 3 conversation turns to resolve references like "that", "him", "it".
...
│   │   ├── brain_service.py     # Astro: Groq classifier (general vs realtime)
...
### POST `/chat/astro/stream`
Astro mode: brain classifies query → routes to general or realtime. Streaming.
...
| `GROQ_BRAIN_MODEL` | No | `llama-3.1-8b-instant` | Brain classifier (Astro mode) |
| `ASSISTANT_NAME` | No | `Astro` | Assistant's name |
| `ASTRO_USER_TITLE` | No | - | How to address the user (e.g. "Sir"); legacy `JARVIS_USER_TITLE` is also read |
| `ASTRO_OWNER_NAME` | No | - | Owner's name (e.g. Shreshth); helps AI know who it serves (legacy `JARVIS_OWNER_NAME` still supported) |
...
- **Astro** (default): Unified mode. The brain classifies each query and routes to General or Realtime automatically.
...
**ASTRO** was developed by **Shreshth Kaushik**, an online educator, businessman, and programmer known for simplifying complex topics with innovative teaching methods.
...
- **Project updates:** Watch the repository releases or follow Shreshth's channels for ASTRO enhancements and demos.
> Stay current with ASTRO by checking the repo releases or Shreshth's socials (YouTube, Instagram, Telegram).
