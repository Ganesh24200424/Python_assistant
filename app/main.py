...
This is the central FastAPI application file for ASTRO (Just A Rather
...
# "ASTRO" and use it throughout this file (and it's the logger you
...
#   ASSISTANT_NAME         — display name ("ASTRO")
...
#   2025-02-21 14:30:00 | INFO     | ASTRO            | message…
...
def print_title():
    """
    Print the ASTRO ASCII art banner to the console when the server
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # --- Step 3b: Brain Service (Groq query classifier for Astro mode) ---
        ...
        logger.info("=" * 60)
        logger.info("ASTRO is online and ready!")
...
async def api_info():
    ...
    return {
        ...
        "endpoints": {
            ...
            "/chat/astro/stream": "Astro unified route (brain classifies, streams)",
...
async def chat(request: ChatRequest):
    """
    General chat endpoint - send a message to ASTRO.
...
def _stream_generator(session_id: str, chunk_iter, is_realtime: bool, tts_enabled: bool = False):
    ...
    try:
        # ── Main loop: iterate over LLM text chunks (or search_results, activity in Astro) ──
        for chunk in chunk_iter:
            # Astro mode yields activity events for the left-side panel.
            if isinstance(chunk, dict) and "_activity" in chunk:
                yield f"data: {json.dumps({'activity': chunk['_activity']})}\n\n"
                continue
            # Realtime/Astro may yield search_results for the right-side widget.
...
async def chat_realtime(request: ChatRequest):
    """
    Realtime chat endpoint - send a message to ASTRO with Tavily web search.
...
@app.post("/chat/astro/stream")
async def chat_astro_stream(request: ChatRequest):
    """
    Unified Astro route — brain classifies each query, then routes to general
    ...
    if not chat_service:
        raise HTTPException(status_code=503, detail="Service not initialized")
    logger.info("[API /chat/astro/stream] Incoming | session_id=%s | message_len=%d | message=%.100s",
                request.session_id or "new", len(request.message), request.message)
    try:
        session_id = chat_service.get_or_create_session(request.session_id)
        chunk_iter = chat_service.process_astro_message_stream(session_id, request.message)
        return StreamingResponse(
            _stream_generator(session_id, chunk_iter, is_realtime=True, tts_enabled=request.tts),
            media_type="text/event-stream",
            headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
        ...
        if _is_rate_limit_error(e):
            raise HTTPException(status_code=429, detail=RATE_LIMIT_MESSAGE)
        logger.error("[API /chat/astro/stream] Error: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
