"""
BRAIN SERVICE MODULE
====================

Classifies user queries as "general" (no web search) or "realtime" (needs
Tavily search). Uses Groq (llama-3.1-8b-instant) with API key rotation —
brain and chat never use the same key for a single request.

PURPOSE:
  When the user selects "Astro" mode, every message is first sent to
  this brain. The brain inspects the user's message AND the chat history
  to decide which backend to use:
    - general: Questions answerable from knowledge + vector store.
    - realtime: Questions that need current information (news, "latest", etc.).

KEY ROTATION:
  Brain uses key_index from get_next_key_pair(); chat uses the next key.
  This ensures even load distribution across API keys.
"""
...
class BrainService:
    """
    Classifies user queries as general or realtime using Groq.
    Uses one LLM client per API key for rotation (brain and chat use different keys).
    """
    ...
    def classify(
        ...
        """
        Classify the user's message as "general" or "realtime".

        Args:
            user_message: The current message from the user.
            chat_history: Optional list of (user_text, assistant_text) pairs.
            key_index: Which Groq API key to use (from key rotation).

        Returns:
            (query_type, reasoning, elapsed_ms).
        """
        if not self._llms:
            return ("realtime", REASONING_DEFAULT, 0)

        # Build context: last N turns, truncated to avoid token overflow.
        context_lines = []
        if chat_history:
