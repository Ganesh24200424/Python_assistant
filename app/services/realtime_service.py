...
class RealtimeGroqService(GroqService):
    ...
    def _extract_search_query(
        ...
        # Skip extraction for very short queries — use raw (saves ~150–300ms)
        q = question.strip()
    ...
    def search_tavily(self, query: str, num_results: int = 7) -> str:
        ...
        HOW IT WORKS:
          1. Call self.tavily_client.search() with these key parameters:
             - search_depth="fast": Fast search for lower latency (vs "advanced"
               for deeper crawling; "fast" balances speed and quality).
    ...
    def prefetch_web_search(
        ...
        """
        Extract search query and run Tavily search. Returns (formatted_results, payload).
        Used for parallel pre-fetch in Astro mode — run alongside brain classification.
        """
        try:
            t0 = time.perf_counter()
            search_query = self._extract_search_query(question, chat_history)
    ...
    def stream_response_with_prefetched(
        ...
        """
        Stream the realtime response using pre-fetched Tavily results.
        No search is performed — uses the provided formatted_results and payload.
        Used when Astro pre-fetches search in parallel with brain classification.
        Search results are emitted by ChatService immediately after routing.
        """
        try:
            extra_parts = [escape_curly_braces(formatted_results)] if formatted_results else None
            prompt, messages = self._build_prompt_and_messages(
