...
#
# ASTRO_BRAIN_SEARCH_TIMEOUT: Max seconds to wait for brain classification and
#   web search prefetch. Prevents indefinite block if the brain or Tavily hangs.
ASTRO_BRAIN_SEARCH_TIMEOUT = 15
...
class ChatService:
    ...
    def __init__(
        ...
        """
        Initialize the ChatService with its LLM backends and optional brain.
        ...
        Args:
            ...
            brain_service: The Groq-powered brain that classifies queries as
                           general or realtime. Used only for process_astro_message_stream.
                           If None, Astro mode will default to realtime.
        ...
        # Lock to prevent concurrent writes to the same session file.
    ...
    def format_history_for_llm(self, session_id: str, exclude_last: bool = False) -> List[tuple]:
        ...
        while i < len(messages_to_process) - 1:
            ...
            if user_msg.role == "user" and ai_msg.role == "assistant":
                # Found a complete pair — add it. Defensive: ensure content is str.
    ...
    def process_message_stream(
        ...
        # Activity events for left panel (full detail, no truncation)
    ...
    def process_realtime_message_stream(
        ...
        # Activity events for left panel (full detail)
        yield {"_activity": {"event": "query_detected", "message": user_message}}
    ...
    def process_astro_message_stream(
        self, session_id: str, user_message: str
    ) -> Iterator[Union[str, Dict[str, Any]]]:
        """
        Unified Astro route: brain classifies the query, then routes to general
        or realtime stream. Uses the SAME session as general/realtime.

        Yields:
          - {"_activity": {...}} — Events for the left-side activity panel.
          - {"_search_results": ...} — From realtime (Tavily data for right widget).
          - str — Text chunks from the LLM.
        """
        logger.info("[ASTRO-STREAM] Session: %s | User: %.200s", session_id[:12], user_message)
        self.add_message(session_id, "user", user_message)
        self.add_message(session_id, "assistant", "")
        chat_history = self.format_history_for_llm(session_id, exclude_last=True)

        # Activity: query detected (full message)
        yield {"_activity": {"event": "query_detected", "message": user_message}}

        # Key rotation: brain and chat use different keys (1→2→3→4→5...)
        brain_idx, chat_idx = get_next_key_pair(len(GROQ_API_KEYS), need_brain=True)

        # Run brain classification AND web search in parallel (pre-fetch for realtime)
        query_type = "realtime"
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            try:
                query_type, reasoning, brain_elapsed_ms = future_brain.result(timeout=ASTRO_BRAIN_SEARCH_TIMEOUT)
            except FuturesTimeoutError:
                logger.warning("[ASTRO] Brain classification timed out after %ds, defaulting to realtime", ASTRO_BRAIN_SEARCH_TIMEOUT)
                query_type, reasoning, brain_elapsed_ms = "realtime", "Brain timeout, defaulting to realtime", 0

            # Only wait for search when realtime — general can proceed immediately (saves 2–3s)
            if query_type == "general":
                formatted_results, search_payload = "", None
            else:
                try:
                    formatted_results, search_payload = future_search.result(timeout=ASTRO_BRAIN_SEARCH_TIMEOUT)
                except FuturesTimeoutError:
                    logger.warning("[ASTRO] Web search prefetch timed out after %ds", ASTRO_BRAIN_SEARCH_TIMEOUT)
                    formatted_results, search_payload = "", None

        logger.info("[ASTRO] Brain: %s in %d ms — %s", query_type, brain_elapsed_ms, reasoning)
        ...
        # Emit search results immediately when realtime — pre-fetched in parallel with brain
        ...
        try:
            if query_type == "general":
                ...
                # General: do NOT emit search results (we pre-fetched but don't use/show them)
            else:
                if not self.realtime_service:
                    raise ValueError("Realtime service not initialized.")
                # Realtime: use pre-fetched results (no second search)
            ...
            final_response = self.sessions[session_id][-1].content
            logger.info("[ASTRO-STREAM] Completed | Route: %s | Chunks: %d | Response length: %d chars",
