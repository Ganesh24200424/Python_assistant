/* ================================================================
   ASTRO Frontend — Main Application Logic
   ================================================================

   ARCHITECTURE OVERVIEW
   ---------------------
   This file powers the entire frontend of the ASTRO AI assistant.
   It handles:
   ...
   4. ANIMATED ORB — A WebGL-powered visual orb (rendered by a
      ...
      "activates" when ASTRO is speaking and goes idle otherwise.
 ...
 * (e.g., "https://astro.example.com"). During local development, it falls
...
/*
 * currentMode — Which AI pipeline to use: 'astro', 'general', or 'realtime'.
 * - astro:   Unified route; brain classifies, then routes to general or realtime.
...
const btnAstro     = $('btn-astro');       // The "Astro" mode button (unified, brain-routed)
...
const activityPanel       = $('activity-panel');          // Left panel for Astro activity flow
...
function init() {
    if (!chatMessages || !messageInput) {
        console.error('[ASTRO] Required DOM elements (chat-messages, message-input) not found.');
        return;
    }
    loadSettings();
...
function bindEvents() {
    ...
    // MODE TOGGLE BUTTONS — Switch between Astro, General, and Realtime modes
    if (btnAstro)   btnAstro.addEventListener('click',   () => setMode('astro'));
 ...
 *
 * @param {string} mode - Either 'astro', 'general', or 'realtime'.
 *
 * Updates:
...
function setMode(mode) {
    currentMode = mode;
    if (btnAstro)   btnAstro.classList.toggle('active', mode === 'astro');
...
/* ================================================================
   ...
   consists of:
     - An avatar circle ("A" for Astro, "U" for user)
 ...
 *     <div class="msg-body">
 *       <div class="msg-label">Astro (General) | You</div>
...
function addMessage(role, text) {
    ...
    label.textContent = role === 'assistant'
        ? `Astro  (${currentMode === 'astro' ? 'Astro' : currentMode === 'realtime' ? 'Realtime' : 'General'})`
        : 'You';
...
function addTypingIndicator() {
    ...
    const label = document.createElement('div');
    label.className = 'msg-label';
    label.textContent = `Astro  (${currentMode === 'astro' ? 'Astro' : currentMode === 'realtime' ? 'Realtime' : 'General'})`;
...
async function sendMessage(textOverride) {
    ...
    const endpoint = currentMode === 'astro'
        ? '/chat/astro/stream'
        ...
        while (!streamDone) {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    // ACTIVITY — Astro flow (query detected, decision, routing): show in left panel
                    if (data.activity) {
